...
bool HelloWorld::init()
{
    ...
    auto label = Label::createWithSystemFont("测试Box2d", "微软雅黑", 12);
    ...
    label->setString(StringUtils::format("测试Box2d WinSize(%.1f,%.1f) WinSizeInPixels(%.1f, %.1f) %.2f", director->getWinSize().width, director->getWinSize().height, director->getWinSizeInPixels().width, director->getWinSizeInPixels().height, director->getContentScaleFactor()));
